...
Assume karnofsky to have linear relationship and turn it numeric as otherwise we wind up creating a bunuch more variables and mess with the power.
...
```{r best model}
final_model <- coxph(surv_actg ~ tx + ivdrug_bin + karnof + age + base_cd4, data = actg_c)

plot(survfit_na_actg,
     col = 1:2, lwd = 2,
     fun = 'cloglog',
     main = 'clog-log Survival Curves')
legend('topleft', c('SoC', 'SoC + IDV'), col = 1:2, lwd = 2)
```